...
void preProcess(Mat src, Mat &dst)
{
	...
	Canny(imageBlurred, dst, 10, 20, 3);
}
...
int main( int argc, char** argv)
{
  ...
  if(! image.data ) {
...
#ifndef NDEBUG
	imwrite(getOutputFileName(image_name, "edged.jpeg"), edged);
#endif
	...
	if(i < approx.size())
	{
...
#ifndef NDEBUG
		imwrite(getOutputFileName(image_name, "flat.jpeg"), warped);
#endif
		...
		imwrite(getOutputFileName(image_name, "scanned.jpeg"), warped);
	}
}
